...
class PadUpAndDown(DualTransform):
    def __init__(self,  axis ,always_apply=False, p=1):
        super().__init__(always_apply, p)
        self.axis = axis

    def apply(self, img):
        return self.padupdown(img)

    def apply_to_mask(self, mask):
        return self.padupdown(mask)

    def padupdown(self, img):
        if self.axis == 0:
            img = np.concatenate([img[0:1, :, :], img, img[img.shape[0]-1: img.shape[0],:,:]], axis=self.axis)
        elif self.axis == -1 or  self.axis == 2:
            img = np.concatenate([img[:, :, 0:1], img, img[:, :, img.shape[self.axis] - 1: img.shape[self.axis]]], axis=self.axis)
        return img
...
class CropNonEmptyMaskIfExists(DualTransform):
    def __init__(self, shape, always_apply=False, p=1.0, GetParams=False):
        super().__init__(always_apply, p, GetParams=GetParams)
        self.height = shape[0]
        self.width = shape[1]
        self.depth = shape[2]
...
class CropNonEmptyMaskIfExistsBalance(DualTransform):
    def __init__(self, shape, fg_class_number=1, always_apply=False, p=1.0, GetParams=False):
        super().__init__(always_apply, p, GetParams=GetParams)
        self.height = shape[0]
        self.width = shape[1]
        self.depth = shape[2]
        self.fg_class_number = fg_class_number
    def apply(self, img, x_min=0, y_min=0, z_min=0, x_max=0, y_max=0, z_max=0):
        return F.crop(img, x_min, y_min, z_min, x_max, y_max, z_max)
    ...
    def get_params(self, **data):
        ...
        if mask.sum() == 0:
            ...
            middle_point = random.randint(1, self.fg_class_number)
            non_zero = np.where(mask == middle_point)
            if len(non_zero[0]) == 0:
                non_zero = np.argwhere(mask)
            else:
                non_zero = np.asarray(non_zero)
                non_zero = np.transpose(non_zero, axes=[1, 0])
            x, y, z = random.choice(non_zero)
            x_min = x - random.randint(0, self.height - 1)
            y_min = y - random.randint(0, self.width - 1)
            z_min = z - random.randint(0, self.depth - 1)
